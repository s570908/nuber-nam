...
import {
	GetNearbyRides,
	NearbyRideSubscription as RideSubscription,
	nearbyRides_NearbyRideSubscription as RideSubscription_RideSubscription,
	UpdateRideStatus as TakeRequestedRide,
	updateRideVariables as TakeRequestedRideVariables
} from "../../types/api";
...
const DriverHomeContainer: React.FC<IProps> = ({ history }) => {
	...
	useQuery<GetNearbyRides>(GET_NEARBY_RIDES, {
		onCompleted: ({ GetNearbyRides }) => {
			const { ok: res, error, ride: rides } = GetNearbyRides;
			if (res && rides) {
				rides.forEach(ride => {
					if (ride) {
						rideQueue.push(ride);
